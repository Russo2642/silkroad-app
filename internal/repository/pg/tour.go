...
func (r *TourPostgres) Create(tour tour.Tour) (int, error) {
	...
	baseSlug := slug.Make(tour.Title)
	tour.Slug = baseSlug

	suffix := 1
	for {
		var existingID int
		checkSlugQuery := fmt.Sprintf("SELECT id FROM %s WHERE slug = $1", tourTable)
		err := r.db.QueryRow(checkSlugQuery, tour.Slug).Scan(&existingID)
		...
		tour.Slug = fmt.Sprintf("%s-%d", baseSlug, suffix)
		suffix++
	}
...
func (r *TourPostgres) GetTourByField(field, value string) (tour.Tour, error) {
	var t tour.Tour
	var descriptionRouteJSON []byte
